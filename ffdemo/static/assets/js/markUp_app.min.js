...
e.fn.showLoader(e.fn.getString("loading-marks-msg"),"overlay-light");a.ajax({url:g,data:b,dataType:"JSON"}).success(f).success(function(){e.fn.hideLoader()})},setupMarks:function(a,b){var c=a.modules.linear;if(b.length!=0){for(var f=0;f<b.length;f++)b[f].reference in c.marks&&b.shift(f,1);if(b.length!=0){f=[];for(var g in c.marks)f.push([g,c.marks[g].id]);f.sort(function(a,b){return a[1]-b[1]});var h=f.length==0||f[0][1]<b[0].id?c.rightBuffer:c.leftBuffer,i=h==c.leftBuffer?!0:!1;i&&b.reverse();var j=
h.length>0?c.marks[h[i?0:h.length-1]]:c.scene.objects[i?0:c.scene.objects.length-1];for(f=0;f<b.length;f++)if(!(b[f].reference in c.marks)){var k=JSON.parse(b[f].points_obj_simplified);if(!(k==null||!("strokes"in k)||k.strokes.length==0||k.strokes[0].length<2)){g=new Mark.gmlMark(k.strokes,b[f].reference,b[f].country_code,b[f].date_drawn,k.rtl,b[f].id,b[f].is_approved);if(b[f].contributor)g.contributor_name=b[f].contributor,g.extra_info=k.extra_info,g.contributor_url=k.contributor_url,g.color="0,139,211";
c.marks[g.reference]=g;j&&g.positionRelativeTo(j,i);i?h.unshift(g.reference):h.push(g.reference);j=g}}}}},refillBuffer:function(a,b){var c=a.modules.linear;if(!c.requestingMarks){c.requestingMarks=!0;var f=b==c.leftBuffer,g=null;g=b.length>0?c.marks[b[f?0:b.length-1]]:c.scene.objects[f?0:c.scene.objects.length-1];modules.linear.fn.loadMarks(a,{reference:g.reference,include_forward:f?0:20,include_back:f?20:0,include_mark:0,success:function(b){b.success?(modules.linear.fn.setupMarks(a,b.marks),b.marks.length<
18&&(c[f?"moreLeft":"moreRight"]=!1)):c[f?"moreLeft":"moreRight"]=!1;c.requestingMarks=!1}})}},updateBuffers:function(a,b,c){var f=a.modules.linear;if(f.scene.objects.length!=0){for(var g=f.scene.objects[0];g&&g.position.x+g.bWidth<b;)f.leftBuffer.push(f.scene.objects.shift().reference),g=f.scene.objects[0];for(g=f.scene.objects[f.scene.objects.length-1];g&&g.position.x>c;)f.rightBuffer.unshift(f.scene.objects.pop().reference),g=f.scene.objects[f.scene.objects-1];f.leftBuffer.length<5&&f.scene.objects.length>
0&&f.moreLeft&&!f.requestingMarks?modules.linear.fn.refillBuffer(a,f.leftBuffer):f.rightBuffer.length<5&&f.scene.objects.length>0&&f.moreRight&&!f.requestingMarks&&modules.linear.fn.refillBuffer(a,f.rightBuffer)}},updateScene:function(a,b,c){a=a.modules.linear;if(a.rightBuffer.length>0)for(var f=a.marks[a.rightBuffer[0]];f&&f.position&&f.position.x<c;)a.scene.objects.push(a.marks[a.rightBuffer.shift()]),f=a.rightBuffer[0];if(a.leftBuffer.length>0)for(f=a.marks[a.leftBuffer[a.leftBuffer.length-1]];f&&
f.position&&f.position.x+f.bWidth>b;)a.scene.objects.unshift(a.marks[a.leftBuffer.pop()]),f=a.leftBuffer[a.leftBuffer.length-1]},jumpToMark:function(a,b,c){var f=a.modules.linear;f.currentMark=f.marks[b];if(c){var g=(new Date).getTime()*2;f.scene.timers[b]={start:g,end:g+f.currentMark.maxTime,speed:1}}modules.linear.fn.centerCurrentMark(a,function(){c&&modules.linear.fn.replayCurrentMark(a);modules.linear.fn.showMarkInformation(a)})},closestMark:function(a){a=a.modules.linear;for(var b=a.scene.objects[0],
c=1;c<a.scene.objects.length;c++){var f=a.scene.objects[c];Math.abs(a.scene.camera.position.x-(f.position.x+f.bWidth/2))<Math.abs(a.scene.camera.position.x-(b.position.x+b.bWidth/2))&&(b=f)}return b},hoverTest:function(a,b,c,f){if(f&&f.renderedBounds&&b>=f.renderedBounds.minX&&b<=f.renderedBounds.maxX&&c>=f.renderedBounds.minY&&c<=f.renderedBounds.maxY)return f;return modules.linear.fn.hitTest(a,b,c)},hitTest:function(a,b,c){a=a.modules.linear;for(var f=0;f<a.scene.objects.length;f++)if(a.scene.objects[f].renderedBounds&&
...
a("#location-dialog").fadeOut();return!1});c.invite_code&&c.contributor_type=="t"?(c.captureLimit=1E3,c.$capture.addClass("translator"),a("#translator-fields").find("#translator-locale").text("'"+b.locale+"'").end().show().collapsibleMod().hide().fadeIn("slow")):c.invite_code&&c.contributor_type=="c"&&(c.$capture.addClass("contributor"),a("#contributor-fields").show().collapsibleMod().hide().fadeIn("slow"))},endIntro:function(a){a.app.setLocation("#/mark/new")},locationDialogToggle:function(b){b.preventDefault();
...
b.strokeStyle="rgba(0,0,0,0.2)";b.lineWidth=1;b.beginPath();b.dashedLineTo(f,g,c.rtl?0:c.layerManager.layers.liveDrawingLayer.canvas.width,g,[7,5]);b.closePath();b.stroke();a("#markapp").css({cursor:"default"})},submit:function(b){var c=b.modules.capture;if(c.state!="submitting"){c.state!="preview"&&e.capture.fn.closeShop(b);c.state="submitting";a("#markmaker-submit a").addClass("disabled");var f={};f.rtl=c.rtl;f.strokes=c.strokes;f=JSON.stringify(f);var g=JSON.stringify(c.mark),h=a("#markmaker-country").val()==
"label"?"":a("#markmaker-country").val();b.fn.showLoader(b.fn.getString("submitting-mark"));var i={points_obj:f,points_obj_simplified:g,country_code:h};if(c.invite_code&&c.contributor_type=="t")i.contributor_locale=b.locale,i.invite=c.invite_code;else if(c.invite_code&&c.contributor_type=="c")i.contributor=a("#contributor-name").val(),c.mark.extra_info=a("#contributor-quote").val(),c.mark.contributor_url=a("#contributor-url").val(),i.points_obj_simplified=JSON.stringify(c.mark),i.invite=c.invite_code;
a.ajax({url:"/requests/save_mark",data:i,type:"POST",dataType:"JSON",success:function(a){i.contributor_locale?b.app.setLocation("#/linear/"):(b.fn.storeData("userMark",{reference:a.mark_reference,country_code:h}),b.app.setLocation("#/linear/"+a.mark_reference+"?playback=true"));b.fn.hideLoader()},error:function(){b.fn.showError(b.fn.getString("submit-error"))}});return!1}},drawStroke:function(a,c){var e=a.modules.capture;Mark.thickBrush(e.layerManager.layers.drawnLayer.context,[c]);e.layerManager.layers.drawnLayer.context.fillStyle=
"rgba(255,255,255,0.3)";e.layerManager.layers.drawnLayer.context.strokeStyle="rgba(255,255,255,0.3)";Mark.circleBrush(e.layerManager.layers.drawnLayer.context,[c])},drawGuide:function(a,c,e,g,h){a.strokeStyle="rgba(0,0,0,0.2)";a.lineWidth=1;a.beginPath();a.dashedLineTo(c,e,g,h,[7,5]);a.closePath();a.stroke()},drawCursor:function(a,c,e,g){a.strokeStyle="#ff5400";a.fillStyle="#000000";a.beginPath();a.moveTo(c,e);a.lineTo(c+1,e-8);a.lineTo(c+20,e-27);a.lineTo(c+23,e-23);a.lineTo(c,e);a.closePath();a.stroke();
g*=18.5;g+=4.5;a.beginPath();a.moveTo(c,e);a.lineTo(c+1,e-8);a.lineTo(c+(g-3),e-(g+4));a.lineTo(c+g,e-g);a.lineTo(c,e);a.closePath();a.fill()},commonLoop:function(a){var c=a.modules.capture;c.layerManager.layers.liveDrawingLayer.clean();a.mouseIn&&(c.state=="drawing"||c.state=="intro")&&e.capture.fn.drawCursor(c.layerManager.layers.liveDrawingLayer.context,a.mouseX,a.mouseY,(c.captureLimit-c.capturedPoints)/c.captureLimit)},introLoop:function(){},drawLoop:function(b){var c=b.modules.capture;if(c.currentStroke&&
c.currentStroke.length>0)Mark.thickBrush(c.layerManager.layers.liveDrawingLayer.context,[c.currentStroke]),c.layerManager.layers.liveDrawingLayer.context.fillStyle="rgba(255,255,255,0.3)",c.layerManager.layers.liveDrawingLayer.context.strokeStyle="rgba(255,255,255,0.3)",Mark.circleBrush(c.layerManager.layers.liveDrawingLayer.context,[c.currentStroke]);if(b.mouseIn&&!b.mouseDown){var f,g;c.strokes.length==0?(f=b.mouseX>a(window).width()/2?c.layerManager.layers.liveDrawingLayer.canvas.width:0,g=b.mouseY):
(f=c.lastX,g=c.lastY);e.capture.fn.drawGuide(c.layerManager.layers.liveDrawingLayer.context,f,g,b.mouseX,b.mouseY)}}}}})(jQuery);
