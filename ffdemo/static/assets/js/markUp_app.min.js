...
new a.simplification.Line(d[0],d[d.length-1]),f=0,g=0,h=1;h<=d.length-2;h++){var i=e.distanceToPoint(d[h]);i>f&&(f=i,g=h)}f>=b?(f=d[g],e.distanceToPoint(f,!0),c=c.concat(a.simplification.douglasPeucker(d.slice(0,g+1),b)),c=c.concat(a.simplification.douglasPeucker(d.slice(g,d.length),b))):(f=d[g],e.distanceToPoint(f,!0),c=[d[0]]);return c},simplifyPath:function(d,b){var c=[];c.push(d.shift());c=c.concat(a.simplification.douglasPeucker(d,b));c.push(d[d.length-2]);c.push(d[d.length-1]);return c},weightPath:function(d,
b){for(var c=b.length+1,e=d;tolerance=b.shift();){e=a.simplification.douglasPeucker(e,tolerance);for(var f=0;f<e.length;f++)e[f].significance++}d[0].significance=c;d[d.length-1].significance=c;return d}};return a}(Mark||{});
