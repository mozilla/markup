...
b){return a[1]-b[1]});var g=e.length==0||e[0][1]<b[0].id?c.rightBuffer:c.leftBuffer,h=g==c.leftBuffer?!0:!1;h&&b.reverse();var i=g.length>0?c.marks[g[h?0:g.length-1]]:c.scene.objects[h?0:c.scene.objects.length-1];for(e=0;e<b.length;e++)if(!(b[e].reference in c.marks)){var j=JSON.parse(b[e].points_obj_simplified);if(!(j==null||!("strokes"in j)||j.strokes.length==0||j.strokes[0].length<2)){f=new Mark.gmlMark(j.strokes,b[e].reference,b[e].country_code,b[e].date_drawn,j.rtl,b[e].id,b[e].is_approved);
...
c;)a.scene.objects.push(a.marks[a.rightBuffer.shift()]),e=a.rightBuffer[0];if(a.leftBuffer.length>0)for(e=a.marks[a.leftBuffer[a.leftBuffer.length-1]];e&&e.position&&e.position.x+e.bWidth>b;)a.scene.objects.unshift(a.marks[a.leftBuffer.pop()]),e=a.leftBuffer[a.leftBuffer.length-1]},jumpToMark:function(a,b,c){var e=a.modules.linear;e.currentMark=e.marks[b];if(c){var f=(new Date).getTime()*2;e.scene.timers[b]={start:f,end:f+e.currentMark.maxTime,speed:1}}modules.linear.fn.centerCurrentMark(a,function(){c&&
modules.linear.fn.replayCurrentMark(a);modules.linear.fn.showMarkInformation(a)})},closestMark:function(a){a=a.modules.linear;for(var b=a.scene.objects[0],c=1;c<a.scene.objects.length;c++){var e=a.scene.objects[c];Math.abs(a.scene.camera.position.x-(e.position.x+e.bWidth/2))<Math.abs(a.scene.camera.position.x-(b.position.x+b.bWidth/2))&&(b=e)}return b},hoverTest:function(a,b,c,e){if(e&&e.renderedBounds&&b>=e.renderedBounds.minX&&b<=e.renderedBounds.maxX&&c>=e.renderedBounds.minY&&c<=e.renderedBounds.maxY)return e;
return modules.linear.fn.hitTest(a,b,c)},hitTest:function(a,b,c){a=a.modules.linear;for(var e=0;e<a.scene.objects.length;e++)if(a.scene.objects[e].renderedBounds&&b>=a.scene.objects[e].renderedBounds.minX&&b<=a.scene.objects[e].renderedBounds.maxX&&c>=a.scene.objects[e].renderedBounds.minY&&c<=a.scene.objects[e].renderedBounds.maxY)return a.scene.objects[e];return!1},showMarkInformation:function(d){var b=d.modules.linear;if(!b.currentMark)return!1;var c=b.currentMark;a("#mark-id").text(c.id);var e=
new Date(c.time);e=new Date(e.getTime()+36E5*d.timezoneOffset);var f=[];f.push(e.getHours()+":"+(String(e.getMinutes()).length==1?"0"+e.getMinutes():e.getMinutes()));f.push("GMT");f.push(d.fn.getString("month-abbreviations",e.getMonth())+" "+e.getDate()+", "+e.getFullYear());c.country_code?d.fn.withCountryCodes(function(b){c.country_code in b?a("#mark-country").text(" / "+b[c.country_code]):a("#mark-country").text("")}):a("#mark-country").text("");c.contributor_name?(a("#mark-contributor-name").text(c.contributor_name),
a("#contributor-name").text(c.contributor_name),a("#mark-flag").hide(),"extra_info"in c&&c.extra_info!=null&&c.extra_info!=""&&a("#contributor-quote").text("\u201c"+c.extra_info+"\u201d"),"contributor_url"in c&&d.fn.validate.url(c.contributor_url)?a("#mark-contributor-url").text(c.contributor_url.replace(/(ftp|http|https):\/\//,"").replace(/\/$/,"")).attr("href",c.contributor_url):a("#mark-contributor-url").text("").attr("href",""),a("#contributor-information").show()):(a("#mark-contributor-name, #contributor-quote, #contributor-name, #mark-contributor-url").text(""),
a("#contributor-information").hide(),a("#mark-flag").show());a("#mark-timestamp").text(f.join(" "));a("#url-share input").val(window.location.href);if(b.linear_root!="moderate")a("#twitter-share").data("socialShare-context").share_params.url=window.location.href,a("#facebook-share").data("socialShare-context").share_params.u=window.location.href;b.linear_root=="moderate"&&a("#approve-mark-checkbox").attr("checked",c.is_approved);b.currentMark!=null&&b.currentMark.reference in b.flaggings?a("#mark-flag").addClass("disabled"):
...
a.modules.linear;a.currentMark!=null&&a.currentMark.reference&&window.open("/gml/"+a.currentMark.reference)},flagCurrentMark:function(d){var b=d.modules.linear;b.currentMark!=null&&b.currentMark.reference in b.flaggings||a.ajax({url:"/requests/flag_mark",data:{reference:b.currentMark.reference},type:"POST",dataType:"JSON",success:function(){a("#mark-flag").addClass("disabled");b.flaggings[b.currentMark.reference]=!0;d.fn.storeData("markFlaggings",b.flaggings)},error:function(){d.fn.showError(d.fn.getString("error-msg"),
"#/linear/")}})},deleteCurrentMark:function(d){var b=d.modules.linear,c=b.currentMark.reference;a.ajax({url:"/requests/delete_mark",data:{reference:c},type:"POST",dataType:"JSON",success:function(){modules.linear.fn.removeMarkFromScreen(d,c);b.currentMark=null;modules.linear.fn.hideMarkInformation(d)},error:function(){d.fn.showError(d.fn.getString("error-msg"),"#/linear/")}})},approveCurrentMark:function(d,b){var c=d.modules.linear,e=c.currentMark.reference;a.ajax({url:"/requests/approve_mark",data:{reference:e,
should_approve:b},type:"POST",dataType:"JSON",success:function(){modules.linear.fn.removeMarkFromScreen(d,e);c.currentMark=null;modules.linear.fn.hideMarkInformation(d)},error:function(){d.fn.showError(d.fn.getString("error-msg"),"#/linear/")}})},removeMarkFromScreen:function(a,b){for(var c=a.modules.linear,e=null,f=0;f<c.scene.objects.length;f++)if(e||c.scene.objects[f].reference==b)e?f==0?c.scene.objects[f].positionToStart():c.scene.objects[f].positionRelativeTo(c.scene.objects[f-1]):(e=f,c.scene.objects.splice(f,
1));c.eventChange=!0},centerCurrentMark:function(a,b){var c=a.modules.linear;if(!c.currentMark)return!1;modules.linear.fn.showMarkInformation(a);c.cameraChange.aX=0;c.cameraChange.vX=0;c.cameraChange.aZ=0;c.cameraChange.vZ=0;var e=Math.abs(c.currentMark.position.x-c.scene.camera.position.x)/2;e=Math.max(1E3,e);var f="cameraEase"in c.tweens?c.tweens.cameraEase:new TWEEN.Tween(c.scene.camera.position);f.to({x:c.currentMark.position.x+c.currentMark.bWidth/2,y:c.currentMark.position.y+c.currentMark.bHeight/
2,z:c.currentMark.position.z-1E3},e).onComplete(function(){delete c.tweens.cameraEase;typeof b==="function"&&b(this)}).easing(e>1200?TWEEN.Easing.Quadratic.EaseInOut:TWEEN.Easing.Quartic.EaseInOut).start();c.tweens.cameraEase=f}}}})(jQuery);
...
Mark.renderer.renderScene(b.vizScene,{width:a.width,height:a.height});if(b.curLocaleMark&&(b.layerManager.layers.mainMark.clean(),b.curLocaleMark)){var c=500/b.curLocaleMark.bWidth;Mark.renderer.renderMark(b.layerManager.layers.mainMark.context,b.curLocaleMark,{offset:{x:a.width/2-115,y:a.height-240},scale:{x:c,y:c,thickness:c},color:"255,84,0",timer:b.textScene.timers[b.curLocaleMark.reference]})}},ready:function(d){a("#markmaker").hide().children().hide();a.when(modules.intro.fn.initInterface(d),
modules.intro.fn.loadMarks(d)).then(function(){modules.intro.fn.runVizPreview(d)}).then(function(){modules.intro.fn.startDomAnimation(d)}).fail(function(){modules.intro.fn.simpleIntro(d)})}},fn:{init:function(d,b){var c=d.modules.intro;if(c.initialized)for(option in a.extend(c,c,b),modules.intro.defaults)b[option]==null&&(c[option]=modules.intro.defaults[option]);else a.extend(c,modules.intro.defaults,b),a.extend(c,c,modules.intro.config),c.$intro=a("<div />").addClass("intro-container"),d.$container.append(c.$intro),
c.vizScene=new Mark.scene,c.textScene=new Mark.scene,c.layerManager=new Mark.layerManager(c.$intro.get(0)),c.layerManager.addLayer("viz"),c.vizScene.canvasContext=c.layerManager.layers.viz.context,c.layerManager.addLayer("mainMark"),c.textScene.canvasContext=c.layerManager.layers.mainMark.context,c.layerManager.addLayer("X"),d.fn.trigger("resize"),c.initialized=!0},deinit:function(a){var b=a.modules.intro;b.$intro.fadeOut("fast",function(){b.layerManager.removeAll();b.$intro.remove();b.initialized=
!1})},initInterface:function(d){a("#markmaker").unbind("resize.markApp").bind("resize.markApp",function(b,c,d){b=c/2+Math.min(500,c/2-20);var f=d-140,g=!1;a("#markmaker").css("background-position","0 "+f+"px");a("#markmaker").is(":visible")||(g=!0,a("#markmaker, #browse-marks, #click-anywhere, #intro-main-copy").css({display:"block"}));a("#browse-marks").css({top:f-50,left:b-85});a("#click-anywhere").css({top:f+12,left:b-a("#intro-main-copy").width()});a("#intro-main-copy").css({top:f-a("#intro-main-copy").height()-
100,left:b-a("#intro-main-copy").width()});g&&a("#markmaker, #browse-marks, #click-anywhere, #intro-main-copy").css({display:"none"});xHeight=d+d/5;xWidth=xHeight*1.545;a("#the-big-x").css({top:-(d/5),bottom:-(d/5),left:c/2-xWidth+(xHeight-500)})}).trigger("resize.markApp",[d.width,d.height]).width(0).height(d.height)},loadMarks:function(d){d.fn.showLoader(d.fn.getString("loading-intro-msg"),"overlay-light");return a.ajax({url:"/requests/get_translated_marks",dataType:"JSON"}).success(function(a){modules.intro.fn.setupMarks(d,
a.marks);d.fn.hideLoader()}).error(function(){d.fn.hideLoader()})},setupMarks:function(d,b){var c=d.modules.intro;if(!(typeof b==="undefined"||b.length==0)){a(c.layerManager.layers.viz.canvas).css("opacity",0);b.sort(function(){return Math.round(Math.random())-0.5});for(var e=null,f=0;f<b.length;f++)try{var g=JSON.parse(b[f].points_obj_simplified),h=new Mark.gmlMark(g.strokes,b[f].reference,b[f].country_code,b[f].date_drawn,g.rtl,b[f].id,b[f].is_approved);if(!c.currentMark)c.currentMark=h;b[f].reference in
c.marks||(c.marks[h.reference]=h);e&&h.positionRelativeTo(e,!1);if(b[f].contributor_locale==d.locale||!c.curLocaleMark){var i=(new Date).getTime()*3;c.curLocaleMark=new Mark.gmlMark(g.strokes,b[f].reference,b[f].country_code,b[f].date_drawn,g.rtl,b[f].id,b[f].is_approved);c.textScene.timers[c.curLocaleMark.reference]={start:i,end:i+c.curLocaleMark.maxTime,speed:2}}c.vizScene.objects.push(h);e=h}catch(j){}}},runVizPreview:function(d){var b=d.modules.intro;if(!(b.vizScene.objects.length<3)){b.vizScene.camera.position.x=
-2E3;b.vizScene.camera.position.z=-3E3;d=b.vizScene.objects[b.vizScene.objects.length-2];var c=new TWEEN.Tween(b.vizScene.camera.position);c.to({x:d.position.x+d.bWidth/2,y:d.position.y+d.bHeight/2,z:d.position.z-2E3},6E3).onComplete(function(){delete b.tweens.cameraEase}).easing(TWEEN.Easing.Quartic.EaseInOut).start();b.tweens.cameraEase=c;a(b.layerManager.layers.viz.canvas).animate({opacity:"1"},"slow").delay(2E3).animate({opacity:"0.1"},"slow");a("#markmaker").delay(3E3)}},startDomAnimation:function(d){a("#markmaker").width(0).show().animate({width:d.width},
"slow",function(){modules.intro.fn.startMarkAnimations(d);a("#intro-main-copy").fadeIn("slow");a("#click-anywhere").delay(200).fadeIn("slow");a("#browse-marks").delay(100).fadeIn("slow")});setTimeout(function(){d.$cursorTooltip=a("#cursor-tooltip").find("p").text(d.fn.getString("cursor-tooltip-intro-msg")).end().css({top:-400}).fadeIn("fast")},3E3)},startMarkAnimations:function(d){d=d.modules.intro;a("#the-big-x").fadeIn("slow");if(d.curLocaleMark){var b=(new Date).getTime()+2E3;d.textScene.timers[d.curLocaleMark.reference]=
{start:b,end:b+d.curLocaleMark.maxTime,speed:2}}},simpleIntro:function(d){a("#markmaker").width(0).show().animate({width:d.width},"slow",function(){a("#intro-main-copy").fadeIn("slow");a("#the-big-x").fadeIn("slow");a("#click-anywhere").delay(200).fadeIn("slow");a("#browse-marks").delay(100).fadeIn("slow")})}}}})(jQuery);
