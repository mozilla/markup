...
j,k)},loop:function(){setTimeout(function(){b.fn.loop()},42);b.frameCount++;b.fn.trigger("loop",{},[])},withCountryCodes:function(g){"US"in b.countries?g(b.countries):a.ajax({url:"/media/assets/js/vendor/country_codes.json",dataType:"JSON",success:function(j){b.countries=j;for(var k=0;k<j.length;k++)b.countries[j[k].code]=j[k].name;g(b.countries)},error:function(){}})},showLoader:function(g,j){j=typeof j==="string"?j:"";g=typeof g==="string"?g:b.fn.getString("default-loading-msg");var k=a("<div />").width(b.width).height(b.height).hide().addClass("overlay-wrapper autoResize").addClass(j).attr("id",
"markapp-loader").append(a("<div />").text(g));b.$container.append(k);k.fadeIn("fast")},hideLoader:function(){a("#markapp-loader").fadeOut("fast",function(){a(this).remove()})},showError:function(g){g=typeof g==="string"?g:b.fn.getString("default-error-msg");g=a("<div />").width(b.width).height(b.height).hide().click(function(j){j.preventDefault();b.fn.hideError()}).addClass("overlay-wrapper autoResize").attr("id","markapp-error").append(a("<div />").attr("id","markapp-error-content").append(a("<p />").html(g)));
b.$container.append(g);g.fadeIn("fast")},hideError:function(){a("#markapp-error").fadeOut("fast",function(){a(this).remove()})},localizeDate:function(g){d=new Date;utc=g.getTime()-d.getTimezoneOffset()*6E4;return nd=new Date(g.getTime()+36E5*b.timezoneOffset)},loadTranslations:function(){a("div.translated-strings").each(function(){a(this).children().each(function(){var g=a(this);if(g.is("ol")){b.translatedStrings[g.attr("id")]=[];g.children().each(function(){b.translatedStrings[g.attr("id")].push(a(this).html())})}else b.translatedStrings[g.attr("id")]=
g.html()})})},getString:function(g,j){return g in b.translatedStrings?typeof b.translatedStrings[g]==="object"&&typeof j==="number"?b.translatedStrings[g][j]:b.translatedStrings[g]:g},storeData:function(g,j){if(typeof localStorage!="undefined")try{if(typeof j==="object")j=JSON.stringify(j);localStorage.setItem(g,j)}catch(k){}},getData:function(g){if(typeof localStorage!="undefined")if(g=localStorage.getItem(g)){if(g[0]=="{")g=JSON.parse(g);return g}else return false},validate:{url:function(g){return/(ftp|http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/.test(g)},
string:function(g){return typeof g==="string"&&g.length!=""?true:false}}}};a(window).delayedBind(300,"resize",function(g){return b.fn.trigger("resize",g)}).bind("keydown keypress keyup",function(g){return b.fn.trigger(g.type,g)}).trigger("resize");b.$container.bind("mousemove mousedown mouseup mouseover mouseout ready swap",function(g){return b.fn.trigger(g.type,g)});b.fn.loop()}var e=a.makeArray(arguments);if(e.length>0){var h=e.shift();if(h in b.public_fn)b.public_fn[h](b,typeof e[0]=="undefined"?
{}:e[0])}return c.data("markApp-context",b)}})(jQuery);
...
15,top:c.mouseY-a("#contributor-quote-box").height()-15});else{a("#contributor-quote-box:visible").fadeOut("fast");b.hoverMark.color="255,111,40"}}else if(b.hoverMark){b.hoverMark.color=b.hoverMark.contributor_name?"0,139,211":"0,0,0";b.hoverMark=null;a("#contributor-quote-box:visible").fadeOut("fast")}},loop:function(c){var b=c.modules.linear,e=b.layerManager.layers.drawingLayer,h={x:b.scene.camera.position.x,y:b.scene.camera.position.y,z:b.scene.camera.position.z};if("cameraEase"in b.tweens)TWEEN.update();
else{b.cameraChange.vZ+=b.cameraChange.aZ;b.cameraChange.vX+=b.cameraChange.aX;b.cameraChange.vX*=0.93;b.cameraChange.vZ*=0.93;b.scene.camera.position.x+=b.cameraChange.vX;b.scene.camera.position.z+=b.cameraChange.vZ;var g=modules.linear.fn.closestMark(c);if(g){var j=b.scene.camera.position.y-(g.position.y+g.bHeight/2);if(j!=0&&Math.abs(j)>=10)b.scene.camera.position.y+=j>0?-10:10}}if(!b.eventChange&&h.x==b.scene.camera.position.x&&h.y==b.scene.camera.position.y&&h.z==b.scene.camera.position.z)return false;
if(b.hoverMark&&!modules.linear.fn.hoverTest(c,c.mouseX,c.mouseY,b.hoverMark)){b.hoverMark.color=b.hoverMark.contributor_name?"0,139,211":"0,0,0";b.hoverMark=null;a("#contributor-quote-box:visible").fadeOut("fast")}e.clean();modules.linear.fn.updateScene(c,b.scene.camera.position.x-1E4,b.scene.camera.position.x+1E4);modules.linear.fn.updateBuffers(c,b.scene.camera.position.x-1E4,b.scene.camera.position.x+1E4);Mark.renderer.renderScene(b.scene,{cursor:{x:c.mouseX,y:c.mouseY},width:c.width,height:c.height});
b.eventChange=false;for(g in b.scene.timers){c=(new Date).getTime();b.scene.timers[g].end<c&&delete b.scene.timers[g];b.eventChange=true}}},fn:{init:function(c,b){var e=c.modules.linear;if("$linear"in c.modules.linear){b.country_code!=e.country_code&&modules.linear.fn.dumpAllMarks(c);a.extend(e,e,b);for(option in modules.linear.defaults)if(b[option]==null)e[option]=modules.linear.defaults[option];modules.linear.fn.updateInterface(c);modules.linear.fn.initMarks(c)}else{a.extend(e,modules.linear.defaults,
b);a.extend(e,e,modules.linear.config);e.$linear=a("<div />").addClass("linear-container");c.$container.append(e.$linear);e.scene=new Mark.scene;e.cameraChange={aX:0,aY:0,aZ:0,vX:0,vY:0,vZ:0};e.layerManager=new Mark.layerManager(e.$linear.get(0));e.layerManager.addLayer("drawingLayer");e.scene.canvasContext=e.layerManager.layers.drawingLayer.context;c.fn.trigger("resize");e.initialized=true}e.flaggings=c.fn.getData("markFlaggings")||{}},deinit:function(c){var b=c.modules.linear;b.$linear.fadeOut("fast",
function(){b.layerManager.removeAll();b.$linear.remove();b.initialized=false})},initInterface:function(c){var b=c.modules.linear;a("#stats").hide();a("#mark-browsing-zoom-in a, #mark-browsing-zoom-out a, #mark-browsing-next a, #mark-browsing-prev a").click(function(e){e.preventDefault()}).bind("mouseup mouseout",function(e){e.preventDefault();if(a(this).data("mouseDown")){if(a(this).is("#mark-browsing-zoom-in a, #mark-browsing-zoom-out a"))c.modules.linear.cameraChange.aZ=0;else if(a(this).is("#mark-browsing-next a, #mark-browsing-prev a"))c.modules.linear.cameraChange.aX=
0;a(this).data("mouseDown",false)}}).bind("mousedown",function(e){e.preventDefault();a(this).data("mouseDown",true);if(a(this).is("#mark-browsing-zoom-in a, #mark-browsing-zoom-out a"))c.modules.linear.cameraChange.aZ=a(this).is("#mark-browsing-zoom-in a")?10:-10;else if(a(this).is("#mark-browsing-next a, #mark-browsing-prev a")){c.modules.linear.cameraChange.aX=a(this).is("#mark-browsing-next a")?10:-10;modules.linear.fn.hideMarkInformation(c)}});c.fn.withCountryCodes(function(e){for(var h=a("#country-select"),
g=0;g<e.length;g++){var j=a("<option />").val(e[g].code).text(e[g].name);h.append(j)}h.change(function(){var k=a(this).val();if(k.length!=2&&b.country_code)c.app.setLocation("#/"+b.linear_root+"/");else k.length==2&&k!=b.country_code&&c.app.setLocation("#/"+b.linear_root+"/country/"+k+"/");a(this).blur();c.$container.focus()});b.country_code&&h.val(b.country_code)});a("#mark-information").hide();a("#mark-playback").click(function(e){e.preventDefault();modules.linear.fn.replayCurrentMark(c)});a("#mark-gml-download").click(function(e){e.preventDefault();
modules.linear.fn.downloadCurrentMark(c)});a("#mark-flag").click(function(e){e.preventDefault();modules.linear.fn.flagCurrentMark(c)});a("#delete-mark").click(function(e){e.preventDefault();modules.linear.fn.deleteCurrentMark(c)});a("#approve-mark-checkbox").change(function(e){e.preventDefault();e=a(this).is(":checked");modules.linear.fn.approveCurrentMark(c,e)});a("#twitter-share").socialShare({share_url:"http://twitter.com/share",share_params:{text:c.fn.getString("twitter-msg")}});a("#facebook-share").socialShare({share_url:"http://www.facebook.com/sharer.php",
share_params:{t:c.fn.getString("facebook-msg")}});modules.linear.fn.updateInterface(c);modules.linear.fn.initMarks(c);a("#sammy #country-select").selectBox({autoWidth:false});a("#sammy #contributor-select").selectBox({autoWidth:false})},initMarks:function(c){var b=c.modules.linear;if(b.reference_mark&&b.reference_mark!="")b.reference_mark in b.marks?modules.linear.fn.jumpToMark(c,b.reference_mark,b.playback):modules.linear.fn.loadMarks(c,{reference:b.reference_mark,include_forward:20,include_back:20,
include_mark:1,success:function(e){if(e.success){modules.linear.fn.setupMarks(c,e.marks);modules.linear.fn.jumpToMark(c,b.reference_mark,b.playback)}else c.fn.showError(b.errorMsg);b.requestingMarks=false}});else modules.linear.fn.loadMarks(c,{offset:0,max:20,success:function(e){if(e.success){modules.linear.fn.setupMarks(c,e.marks);if(firstMark=b.marks[e.marks[0].reference]){b.scene.camera.position.x=-4E3;b.scene.camera.position.z=-3E3;e="cameraEase"in b.tweens?b.tweens.cameraEase:new TWEEN.Tween(b.scene.camera.position);
e.to({x:firstMark.bWidth/2,y:firstMark.bHeight/2,z:-1E3},2E3).onComplete(function(){delete b.tweens.cameraEase;typeof callback==="function"&&callback(this)}).easing(TWEEN.Easing.Quartic.EaseInOut).start();b.tweens.cameraEase=e}}else c.fn.showError(b.errorMsg);b.requestingMarks=false}})},updateInterface:function(c){var b=c.modules.linear,e=c.fn.getData("userMark");e&&(b.country_code?e.country_code==b.country_code:true)?a("#your-mark-link").attr("href","#/"+b.linear_root+"/"+c.fn.getData("userMark").reference).show():
a("#your-mark-link").hide();var h={};if(b.country_code){h.country_code=b.country_code;a("#contributor-select").next().hide();a("#contributor-select-label").hide()}else{a("#contributor-select").next().show();a("#contributor-select-label").show()}a("#mark-browsing-options").is(".country-"+(b.country_code?b.country_code:"all"))||a.ajax({url:"/requests/init_viz_data",data:h,dataType:"JSON",success:function(g){a("#mark-browsing-options").removeAttr("class").addClass("country-"+(b.country_code?b.country_code:
"all"));a("#total-mark-count").text(g.total_marks);if(b.country_code){a("#first-mark-link").attr("href","#/"+b.linear_root+"/country/"+b.country_code+"/"+g.country_first_mark);a("#last-mark-link").attr("href","#/"+b.linear_root+"/country/"+b.country_code+"/"+g.country_last_mark)}else{a("#first-mark-link").attr("href","#/"+b.linear_root+"/"+g.first_mark);a("#last-mark-link").attr("href","#/"+b.linear_root+"/"+g.last_mark)}a("#mark-browsing").collapsibleMod();if(a("#contributor-select option").size()==
1){var j=a("#contributor-select");if(g.contributor_marks)for(var k=0;k<g.contributor_marks.length;k++){var o=a("<option />").val(g.contributor_marks[k].reference).text(g.contributor_marks[k].contributor);j.append(o)}j.change(function(){var q=a(this).val();q.length!="label"&&c.app.setLocation("#/"+b.linear_root+"/"+q);a(this).blur();c.$container.focus()});if(b.country_code){h.country_code=b.country_code;a("#contributor-select").next().hide();a("#contributor-select-label").hide()}else{a("#contributor-select").next().show();
a("#contributor-select-label").show()}}}})},dumpAllMarks:function(c){c=c.modules.linear;for(mark in c.marks)delete c.marks[mark];c.scene.objects=[];c.leftBuffer=[];c.rightBuffer=[];c.moreRight=true;c.moreLeft=true;c.scene.camera.position.x=0;c.scene.camera.position.y=0;c.scene.camera.position.z=-1E3},loadMarks:function(c,b){var e=c.modules.linear,h=b.success;delete b.success;if(e.country_code)b.country_code=e.country_code;var g=b.reference?"/requests/marks_by_reference":"/requests/all_marks";if(e.is_flagged)g=
"/requests/marks_by_flagged";if(b.reference&&!(b.reference in e.marks)){modules.linear.fn.dumpAllMarks(c);c.fn.showLoader(c.fn.getString("loading-marks-msg"),"overlay-light")}else b.reference||c.fn.showLoader(c.fn.getString("loading-marks-msg"),"overlay-light");a.ajax({url:g,data:b,dataType:"JSON"}).success(h).success(function(){c.fn.hideLoader()})},setupMarks:function(c,b){var e=c.modules.linear;if(b.length!=0){for(var h=0;h<b.length;h++)b[h].reference in e.marks&&b.shift(h,1);h=[];for(var g in e.marks)h.push([g,
e.marks[g].id]);h.sort(function(s,r){return s[1]-r[1]});var j=h.length==0||h[0][1]<b[0].id?e.rightBuffer:e.leftBuffer,k=j==e.leftBuffer?true:false;k&&b.reverse();var o=j.length>0?e.marks[j[k?0:j.length-1]]:e.scene.objects[k?0:e.scene.objects.length-1];for(h=0;h<b.length;h++)if(!(b[h].reference in e.marks)){var q=JSON.parse(b[h].points_obj_simplified);if(!(q==null||!("strokes"in q)||q.strokes.length==0||q.strokes[0].length<2)){g=new Mark.gmlMark(q.strokes,b[h].reference,b[h].country_code,b[h].date_drawn,
q.rtl,b[h].id,b[h].is_approved);if(b[h].contributor){g.contributor_name=b[h].contributor;g.extra_info=q.extra_info;g.contributor_url=q.contributor_url;g.color="0,139,211"}e.marks[g.reference]=g;o&&g.positionRelativeTo(o,k);k?j.unshift(g.reference):j.push(g.reference);o=g}}}},refillBuffer:function(c,b){var e=c.modules.linear;if(!e.requestingMarks){e.requestingMarks=true;var h=b==e.leftBuffer,g=null;g=b.length>0?e.marks[b[h?0:b.length-1]]:e.scene.objects[h?0:e.scene.objects.length-1];modules.linear.fn.loadMarks(c,
{reference:g.reference,include_forward:h?0:20,include_back:h?20:0,include_mark:0,success:function(j){if(j.success){modules.linear.fn.setupMarks(c,j.marks);if(j.marks.length==0)e[h?"moreLeft":"moreRight"]=false}else e[h?"moreLeft":"moreRight"]=false;e.requestingMarks=false}})}},updateBuffers:function(c,b,e){var h=c.modules.linear;if(h.scene.objects.length!=0){for(var g=h.scene.objects[0];g&&g.position.x+g.bWidth<b;){h.leftBuffer.push(h.scene.objects.shift().reference);g=h.scene.objects[0]}for(g=h.scene.objects[h.scene.objects.length-
1];g&&g.position.x>e;){h.rightBuffer.unshift(h.scene.objects.pop().reference);g=h.scene.objects[h.scene.objects-1]}if(h.leftBuffer.length<5&&h.scene.objects.length>0&&h.moreLeft&&!h.requestingMarks)modules.linear.fn.refillBuffer(c,h.leftBuffer);else h.rightBuffer.length<5&&h.scene.objects.length>0&&h.moreRight&&!h.requestingMarks&&modules.linear.fn.refillBuffer(c,h.rightBuffer)}},updateScene:function(c,b,e){c=c.modules.linear;if(c.rightBuffer.length>0)for(var h=c.marks[c.rightBuffer[0]];h&&h.position&&
h.position.x<e;){c.scene.objects.push(c.marks[c.rightBuffer.shift()]);h=c.rightBuffer[0]}if(c.leftBuffer.length>0)for(h=c.marks[c.leftBuffer[c.leftBuffer.length-1]];h&&h.position&&h.position.x+h.bWidth>b;){c.scene.objects.unshift(c.marks[c.leftBuffer.pop()]);h=c.leftBuffer[c.leftBuffer.length-1]}},jumpToMark:function(c,b,e){var h=c.modules.linear;h.currentMark=h.marks[b];if(e){var g=(new Date).getTime()*2;h.scene.timers[b]={start:g,end:g+h.currentMark.maxTime,speed:1}}modules.linear.fn.centerCurrentMark(c,
function(){e&&modules.linear.fn.replayCurrentMark(c);modules.linear.fn.showMarkInformation(c)})},closestMark:function(c){c=c.modules.linear;for(var b=c.scene.objects[0],e=1;e<c.scene.objects.length;e++){var h=c.scene.objects[e];if(Math.abs(c.scene.camera.position.x-(h.position.x+h.bWidth/2))<Math.abs(c.scene.camera.position.x-(b.position.x+b.bWidth/2)))b=h}return b},hoverTest:function(c,b,e,h){if(h&&h.renderedBounds&&b>=h.renderedBounds.minX&&b<=h.renderedBounds.maxX&&e>=h.renderedBounds.minY&&e<=
h.renderedBounds.maxY)return h;return modules.linear.fn.hitTest(c,b,e)},hitTest:function(c,b,e){c=c.modules.linear;for(var h=0;h<c.scene.objects.length;h++)if(c.scene.objects[h].renderedBounds&&b>=c.scene.objects[h].renderedBounds.minX&&b<=c.scene.objects[h].renderedBounds.maxX&&e>=c.scene.objects[h].renderedBounds.minY&&e<=c.scene.objects[h].renderedBounds.maxY)return c.scene.objects[h];return false},showMarkInformation:function(c){var b=c.modules.linear;if(!b.currentMark)return false;var e=b.currentMark;
a("#mark-id").text(e.id);var h=new Date(e.time);h=new Date(h.getTime()+36E5*c.timezoneOffset);var g=[];g.push(h.getHours()+":"+(String(h.getMinutes()).length==1?"0"+h.getMinutes():h.getMinutes()));g.push("GMT");g.push(c.fn.getString("month-abbreviations",h.getMonth())+" "+h.getDate()+", "+h.getFullYear());e.country_code?c.fn.withCountryCodes(function(j){e.country_code in j?a("#mark-country").text(" / "+j[e.country_code]):a("#mark-country").text("")}):a("#mark-country").text("");if(e.contributor_name){a("#mark-contributor-name").text(e.contributor_name);
a("#contributor-name").text(e.contributor_name);a("#mark-flag").hide();"extra_info"in e&&e.extra_info!=null&&e.extra_info!=""&&a("#contributor-quote").text("\u201c"+e.extra_info+"\u201d");"contributor_url"in e&&c.fn.validate.url(e.contributor_url)?a("#mark-contributor-url").text(e.contributor_url.replace(/(ftp|http|https):\/\//,"").replace(/\/$/,"")).attr("href",e.contributor_url):a("#mark-contributor-url").text("").attr("href","");a("#contributor-information").show()}else{a("#mark-contributor-name, #contributor-quote, #contributor-name, #mark-contributor-url").text("");
a("#contributor-information").hide();a("#mark-flag").show()}a("#mark-timestamp").text(g.join(" "));a("#url-share input").val(window.location.href);if(b.linear_root!="moderate"){a("#twitter-share").data("socialShare-context").share_params.url=window.location.href;a("#facebook-share").data("socialShare-context").share_params.u=window.location.href}b.linear_root=="moderate"&&a("#approve-mark-checkbox").attr("checked",e.is_approved);b.currentMark.reference in b.flaggings?a("#mark-flag").addClass("disabled"):
...
c.modules.linear;c.currentMark&&c.currentMark.reference&&window.open("/gml/"+c.currentMark.reference)},flagCurrentMark:function(c){var b=c.modules.linear;b.currentMark.reference in b.flaggings||a.ajax({url:"/requests/flag_mark",data:{reference:b.currentMark.reference},type:"POST",dataType:"JSON",success:function(){a("#mark-flag").addClass("disabled");b.flaggings[b.currentMark.reference]=true;c.fn.storeData("markFlaggings",b.flaggings)},error:function(){c.fn.showError(b.errorMsg)}})},deleteCurrentMark:function(c){var b=
c.modules.linear;a.ajax({url:"/requests/delete_mark",data:{reference:b.currentMark.reference},type:"POST",dataType:"JSON",success:function(){var e=b.currentMark.reference,h=null;delete b.marks[b.currentMark.reference];b.currentMark=null;for(var g=0;g<b.scene.objects.length;g++)if(!(!h&&b.scene.objects[g].reference!=e)){if(!h){h=g;b.scene.objects.splice(g,1);b.currentMark=b.scene.objects[g]}g==0?b.scene.objects[g].positionToStart():b.scene.objects[g].positionRelativeTo(b.scene.objects[g-1])}c.app.setLocation("#/"+
b.linear_root+"/"+b.currentMark.reference);b.eventChange=true},error:function(){c.fn.showError(b.errorMs)}})},approveCurrentMark:function(c,b){var e=c.modules.linear;a.ajax({url:"/requests/approve_mark",data:{reference:e.currentMark.reference,should_approve:b},type:"POST",dataType:"JSON",success:function(){e.currentMark.is_approved=b},error:function(){c.fn.showError(e.errorMsg)}})},centerCurrentMark:function(c,b){var e=c.modules.linear;if(!e.currentMark)return false;modules.linear.fn.showMarkInformation(c);
e.cameraChange.aX=0;e.cameraChange.vX=0;e.cameraChange.aZ=0;e.cameraChange.vZ=0;var h=Math.abs(e.currentMark.position.x-e.scene.camera.position.x)/2;h=Math.max(1E3,h);var g="cameraEase"in e.tweens?e.tweens.cameraEase:new TWEEN.Tween(e.scene.camera.position);g.to({x:e.currentMark.position.x+e.currentMark.bWidth/2,y:e.currentMark.position.y+e.currentMark.bHeight/2,z:e.currentMark.position.z-1E3},h).onComplete(function(){delete e.tweens.cameraEase;typeof b==="function"&&b(this)}).easing(h>1200?TWEEN.Easing.Quadratic.EaseInOut:
TWEEN.Easing.Quartic.EaseInOut).start();e.tweens.cameraEase=g}}}})(jQuery);
...
e.mark.strokes.length>0)for(var h=0;h<e.mark.strokes.length;h++)c.capture.fn.drawStroke(b,e.mark.strokes[h])},mousemove:function(b){b.mouseDown&&b.modules.capture.state=="drawing"&&c.capture.fn.capturePoint(b)},mousedown:function(b){switch(b.modules.capture.state){case "drawing":a("#location-dialog").is(":visible")&&a("#location-dialog").fadeOut("fast");b.modules.capture.mark||c.capture.fn.startMark(b);b.modules.capture.currentStroke||c.capture.fn.startStroke(b);break;case "intro":b.modules.capture.mark||
c.capture.fn.startMark(b);b.modules.capture.currentStroke||c.capture.fn.startStroke(b);c.capture.fn.endIntro(b)}},mouseup:function(b){b.modules.capture.state=="drawing"&&c.capture.fn.endStroke(b)},ready:function(b){var e=b.modules.capture;a("#markmaker").hide().children().hide();a("#markmaker-reset a").addClass("disabled").bind("mousedown",function(h){h.preventDefault();c.capture.fn.reset(b)});a("#markmaker-submit a").addClass("disabled").bind("mousedown",function(h){h.preventDefault();c.capture.fn.submit(b)});
b.fn.withCountryCodes(function(h){for(var g=a("#markmaker-country"),j=0;j<h.length;j++){var k=a("<option />").val(h[j].code).text(h[j].name);g.append(k)}});a("#markmaker-location a").bind("mousedown",{context:b},c.capture.fn.locationDialogToggle);a("#markmaker-information").bind("mouseover",{context:b},c.capture.fn.informationDialogToggle).bind("mouseout",{context:b},c.capture.fn.informationDialogToggle);e.state=="drawing"&&c.capture.fn.initDrawing(b);a("#sammy #markmaker-country").selectBox({autoWidth:false})},
loop:function(b){var e=b.modules.capture;if(e.initialized){e.frameCount++;c.capture.fn.commonLoop(b);switch(b.modules.capture.state){case "drawing":c.capture.fn.drawLoop(b)}}}},fn:{init:function(b,e){var h=b.modules.capture;if("$capture"in h){if("state"in e){c.capture.fn.reset(b);e.state=="drawing"&&b.mouseDown&&b.fn.trigger("mousedown");h.state=e.state;c.capture.fn.initDrawing(b)}}else{a.extend(h,c.capture.defaults);a.extend(h,e);a.extend(h,c.capture.config);h.$capture=a("<div />").addClass("capture-container");
b.$container.css({zIndex:100,cursor:"none"}).append(h.$capture);h.layerManager=new Mark.layerManager(h.$capture.get(0));h.layerManager.addLayer("drawnLayer");h.layerManager.addLayer("liveDrawingLayer");b.fn.trigger("resize");h.initialized=true}},deinit:function(b){var e=b.modules.capture;e.$capture.fadeOut("fast",function(){e.layerManager.removeAll();e.$capture.remove();e.initialized=false})},initIntro:function(){},initDrawing:function(b){var e=b.modules.capture;if(a("#browse-marks").is(":visible")){a("#browse-marks, #click-anywhere, #intro-main-copy").fadeOut("fast");
...
a("#contributor-fields").show().collapsibleMod().hide().fadeIn("slow")}},endIntro:function(b){b.app.setLocation("#/mark/new")},locationDialogToggle:function(b){b.preventDefault();a("#location-dialog").is(":visible")?a("#location-dialog").fadeOut("fast"):a("#location-dialog").fadeIn("fast").css({bottom:a("#markmaker-location").height()+25,left:a("#markmaker-location").offset().left+32})},informationDialogToggle:function(b){b.preventDefault();b.type=="mouseout"?a("#information-dialog").fadeOut("fast"):
a("#information-dialog").fadeIn("fast").css({bottom:a("#markmaker-information").height()+36,left:a("#markmaker-information").offset().left-a("#information-dialog").width()+30})},startMark:function(b){var e=b.modules.capture;e.captureTime=(new Date).getTime();e.rtl=b.mouseX>a(window).width()/2;e.mark=new Mark.gmlMark([],"","",e.captureTime,e.rtl);a("#markmaker-submit a, #markmaker-reset a").removeClass("disabled")},endMark:function(b){b.modules.capture.mark.setupVars()},startStroke:function(b){b=b.modules.capture;
b.currentStroke=[];if(b.strokes.length>0)b.captureTime=(new Date).getTime()-(b.strokes[b.strokes.length-1][b.strokes[b.strokes.length-1].length-1].time+b.timeBetweenStrokes)},endStroke:function(b){var e=b.modules.capture;if(e.currentStroke.length>2){e.strokes.push(e.currentStroke);var h=Mark.simplification.simplifyPath(e.currentStroke,1);h=Mark.simplification.weightPath(h,[5,10,20,40]);e.mark.strokes.push(h);e.cleanedStrokes.push(h);c.capture.fn.drawStroke(b,h);e.capturedPoints-=e.currentStroke.length-
h.length}e.currentStroke=null},capturePoint:function(b){var e=b.modules.capture;if(e.capturedPoints>e.captureLimit){b.fn.trigger("mouseup");c.capture.fn.closeShop(b)}else{var h=(new Date).getTime();b=new Mark.gmlPoint(b.mouseX,b.mouseY,h-e.captureTime,0);if(e.currentStroke.length>0){h=e.currentStroke[e.currentStroke.length-1];b.speed=h.speedToPoint(b);b.setAngleFromPoint(h);b.smoothAgainst(h,0.01)}else e.strokes.length>=1&&c.capture.fn.drawGuide(e.layerManager.layers.drawnLayer.context,e.lastX,e.lastY,
b.x,b.y);e.currentStroke.push(b);e.lastX=b.x;e.lastY=b.y;e.capturedPoints++}},reset:function(b){b=b.modules.capture;b.layerManager.layers.liveDrawingLayer.clean();b.layerManager.layers.drawnLayer.clean();b.capturedPoints=0;b.rtl=null;b.mouseDown=false;b.lastX=null;b.lastY=null;b.strokes=[];b.currentStroke=null;b.mark=null;b.captureTime=null;b.state="drawing";a("#markmaker-submit a, #markmaker-reset a").addClass("disabled");a("#markapp").css({cursor:"none"});a("#markmaker-instructions").fadeIn()},
closeShop:function(b){var e=b.modules.capture;e.state="preview";c.capture.fn.endMark(b);e.layerManager.layers.liveDrawingLayer.clean();b=e.layerManager.layers.liveDrawingLayer.context;var h=e.lastX,g=e.lastY;b.strokeStyle="rgba(0,0,0,0.2)";b.lineWidth=1;b.beginPath();b.dashedLineTo(h,g,e.rtl?0:e.layerManager.layers.liveDrawingLayer.canvas.width,g,[7,5]);b.closePath();b.stroke();a("#markapp").css({cursor:"default"})},submit:function(b){var e=b.modules.capture;if(e.state!="submitting"){e.state!="preview"&&
c.capture.fn.closeShop(b);e.state="submitting";a("#markmaker-submit a").addClass("disabled");var h={};h.rtl=e.rtl;h.strokes=e.strokes;h=JSON.stringify(h);var g=JSON.stringify(e.mark),j=a("#markmaker-country").val()=="label"?"":a("#markmaker-country").val();b.fn.showLoader(b.fn.getString("submitting-mark"));var k={points_obj:h,points_obj_simplified:g,country_code:j};if(e.invite_code&&e.contributor_type=="t"){k.contributor_locale=b.locale;k.invite=e.invite_code}else if(e.invite_code&&e.contributor_type==
"c"){k.contributor=a("#contributor-name").val();e.mark.extra_info=a("#contributor-quote").val();e.mark.contributor_url=a("#contributor-url").val();k.points_obj_simplified=JSON.stringify(e.mark);k.invite=e.invite_code}a.ajax({url:"/requests/save_mark",data:k,type:"POST",dataType:"JSON",success:function(o){if(k.contributor_locale)b.app.setLocation("#/linear/");else{b.fn.storeData("userMark",{reference:o.mark_reference,country_code:j});b.app.setLocation("#/linear/"+o.mark_reference+"?playback=true")}b.fn.hideLoader()},
error:function(){b.fn.showError(b.fn.getString("submit-error"))}});return false}},drawStroke:function(b,e){var h=b.modules.capture;Mark.thickBrush(h.layerManager.layers.drawnLayer.context,[e]);h.layerManager.layers.drawnLayer.context.fillStyle="rgba(255,255,255,0.3)";h.layerManager.layers.drawnLayer.context.strokeStyle="rgba(255,255,255,0.3)";Mark.circleBrush(h.layerManager.layers.drawnLayer.context,[e])},drawGuide:function(b,e,h,g,j){b.strokeStyle="rgba(0,0,0,0.2)";b.lineWidth=1;b.beginPath();b.dashedLineTo(e,
h,g,j,[7,5]);b.closePath();b.stroke()},drawCursor:function(b,e,h,g){b.strokeStyle="#ff5400";b.fillStyle="#000000";b.beginPath();b.moveTo(e,h);b.lineTo(e+1,h-8);b.lineTo(e+20,h-27);b.lineTo(e+23,h-23);b.lineTo(e,h);b.closePath();b.stroke();g*=18.5;g+=4.5;b.beginPath();b.moveTo(e,h);b.lineTo(e+1,h-8);b.lineTo(e+(g-3),h-(g+4));b.lineTo(e+g,h-g);b.lineTo(e,h);b.closePath();b.fill()},commonLoop:function(b){var e=b.modules.capture;e.layerManager.layers.liveDrawingLayer.clean();if(b.mouseIn&&(e.state=="drawing"||
e.state=="intro"))c.capture.fn.drawCursor(e.layerManager.layers.liveDrawingLayer.context,b.mouseX,b.mouseY,(e.captureLimit-e.capturedPoints)/e.captureLimit)},introLoop:function(){},drawLoop:function(b){var e=b.modules.capture;if(e.currentStroke&&e.currentStroke.length>0){Mark.thickBrush(e.layerManager.layers.liveDrawingLayer.context,[e.currentStroke]);e.layerManager.layers.liveDrawingLayer.context.fillStyle="rgba(255,255,255,0.3)";e.layerManager.layers.liveDrawingLayer.context.strokeStyle="rgba(255,255,255,0.3)";
Mark.circleBrush(e.layerManager.layers.liveDrawingLayer.context,[e.currentStroke])}if(b.mouseIn)if(!b.mouseDown){var h,g;if(e.strokes.length==0){h=b.mouseX>a(window).width()/2?e.layerManager.layers.liveDrawingLayer.canvas.width:0;g=b.mouseY}else{h=e.lastX;g=e.lastY}c.capture.fn.drawGuide(e.layerManager.layers.liveDrawingLayer.context,h,g,b.mouseX,b.mouseY)}}}}})(jQuery);
